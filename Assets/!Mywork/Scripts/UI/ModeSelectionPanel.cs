    ...
    private void Start()
    {
        foreach (var mod in Enum.GetNames(typeof(gameModesEnum)))
        {
            ...
            btn.onClick.AddListener(() => GameModeSelected(modeEnm));
                btn.onClick.AddListener(() => UIManager.Instance.ShowAds());
                btn.onClick.AddListener(() => SoundsManager.Instance.ButtonClickedSound2());
            }
        }

    }
    
    public void GameModeSelected(gameModesEnum levelMode)
    {
        switch (levelMode)
        {
